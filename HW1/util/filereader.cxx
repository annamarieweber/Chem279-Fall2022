 ...
 *         the reamaing lines follow the format below:
 ...
 * @param string filename the name of the file containing molecukle data for the cluster
 * 
 **/
Cluster readfile(string &filename) {
	...
	if (infile.is_open()) {
		...
		while (infile >> atomNum >> x_coord >> y_coord >> z_coord) {
		  ...
		  cluster.addAtom(atomsCounted,atomNum, x_coord, y_coord, z_coord, 2.951, 5.29);
		  atomsCounted++;
		}
		...
		if(atomsCounted > expectedAtoms){
		  throw BaseException("AtomCountMismatch: Atom Count provided in input File must match number of atoms listed.");
		} else if(!cluster.allAtomsMatch(79)){
		  throw BaseException("AtomTypeNotAllowed: Only atoms with atommic number: 79 are allowed in cluster.");
		}
		return cluster;
	} else {
